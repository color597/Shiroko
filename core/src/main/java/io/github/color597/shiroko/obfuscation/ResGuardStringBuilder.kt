package io.github.color597.shiroko.obfuscation
...
class ResGuardStringBuilder {
    ...
    private val aToZ = (97..122).shuffled().union((65..90)).map { Char(it).toString() }.shuffled()
    private val numbers = (0..9).shuffled().map { it.toString() }
    private val defaultDictionary = aToZ.union(numbers).union(setOf("_")).shuffled()
